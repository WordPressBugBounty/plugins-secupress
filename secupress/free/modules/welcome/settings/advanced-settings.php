...
$this->add_field( array(
	...
	'helpers'           => array(
		...
		array(
			'type'        => 'force-warning',
			'description' => ! empty( $expert_modules ) ? sprintf( _n( 'One Expert Module is still active. Deactivate it:', 'Some Expert Modules are still active. Deactivate them:', count( $expert_modules ), 'secupress' ) . ' <strong>' . wp_sprintf_l( '%l', $expert_modules ) . '</strong>' ) : '',
		),
	),
) );
